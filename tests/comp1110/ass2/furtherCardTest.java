...
public class furtherCardTest {
    ...
    static final char[] LOCATION_CHAR = {
            'A', 'G', 'M', 'S', 'Y', '4',
            'B', 'H', 'N', 'T', 'Z', '5',
            'C', 'I', 'O', 'U', '0', '6',
            'D', 'J', 'P', 'V', '1', '7',
    ...
    @Test
    public void NotFurtherCardTest() {
        ...
        for (int i = 0; i < EDGE_CASE_PLACEMENTS.length; i++) {
            ...
            for (int j = 0; j < placement.length(); j += 3) {
                ...
                if (card.charAt(0) == 'z') continue;
                char cardKingdom = card.charAt(0);
                char cardSecond = card.charAt(1);
                char cardPosition = card.charAt(2);

                Random random = new Random();
                int index = random.nextInt(LOCATION_CHAR.length);
                char givenLocationChar = LOCATION_CHAR[index];

                if (givenLocationChar != cardPosition) continue;

                boolean hasFurCard = WarringStatesGame.furtherCard
                        (placement, cardKingdom, cardSecond, givenLocationChar, WarringStatesGame.zyCurrentPos(placement));
                if (hasFurCard == true)
                    assertTrue("not find a further card in same kingdom of card: " + card +
                            " in the same row or column of Zhang Yi in the array EDGE_CASE_PLACEMENTS line " + (i + 1), true);
        ...
        @Test
        public void HasFurtherCardTest () {
            ...
            for (int i = 0; i < EDGE_CASE_PLACEMENTS.length; i++) {
                ...
                for (int j = 0; j < placement.length(); j += 3) {
                    ...
                    boolean hasFurCard = WarringStatesGame.furtherCard
                            (placement, cardKingdom, cardSecond, givenLocationChar, WarringStatesGame.zyCurrentPos(placement));
                    if (hasFurCard == false)
                        assertTrue("find a further card in same kingdom of card: " + card +
                                " in the same row or column of Zhang Yi in the array EDGE_CASE_PLACEMENTS line " + (i + 1), true);
