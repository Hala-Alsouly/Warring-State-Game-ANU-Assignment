...
public class quickIndexTest {
    ...
    static final String[] EDGE_CASE_PLACEMENTS = {
            ...
            "g1$a0$c0$e0$e3$d4$b6$a4$g0$b5$a7$b1$z9$e1$d0$f0$f1$b2$c1$d3$a5$b4$a2$c5$d1$a3$c2$d2$c3$f2$a6$c4$b3$b0$a1$e2$",
    ...
    @Test
    public void hasCardinBoardTest() {

        for (int i = 0; i < EDGE_CASE_PLACEMENTS.length; i++) {
            if(i==1)
                continue;
            String placement = EDGE_CASE_PLACEMENTS[i];
            for (int j = 0; j < placement.length(); j += 3) {
                String card = placement.substring(j, j + 3);
                char test = card.charAt(2);

                assertTrue("Card in the array EDGE_CASE_PLACEMENTS line " + (i + 1) + " is not located in the BOARD (i.e.card's position is illegal)",
                        WarringStatesGame.quickIndex(test) != -1);
            }
    ...
    @Test
    public void notCardinBoardTest() {

            String placement = EDGE_CASE_PLACEMENTS[1];
            for (int j = 0; j < placement.length(); j += 3) {
                String card = placement.substring(j, j + 3);
                char test = card.charAt(2);

                assertTrue("Card in the array EDGE_CASE_PLACEMENTS line 2 is located in the BOARD (i.e.card's position is legal)",
                        WarringStatesGame.quickIndex(test) == -1);
            }
    ...
    @Test
    public void legalRowColumnTest() {
        for (int i = 0; i < EDGE_CASE_PLACEMENTS.length; i++) {
            String placement = EDGE_CASE_PLACEMENTS[i];
            for (int j = 0; j < placement.length(); j += 3) {
                String card = placement.substring(j, j + 3);
                char position = card.charAt(2);

                int row = WarringStatesGame.quickIndex(position) / 10;
                int col = WarringStatesGame.quickIndex(position) % 10;

                //boolean illegalRow =true;
                if (row < 0 || row > 5) {
                    //wrongRow =false;
                    assertTrue("The row of position in the array EDGE_CASE_PLACEMENTS line " + (i + 1) + " is illegal !",
                            true);
                }
                //boolean illegalCol =true;
                if (col < 0 || col > 5) {
                    //wrongCol =false;
                    assertTrue("The column of position in the array EDGE_CASE_PLACEMENTS line " + (i + 1) + " is illegal !",
                            true);
                }
            }
        }
    ...
    @Test
    public void illegalRowColumnTest() {
            ...
            for (int j = 0; j < placement.length(); j += 3) {
                ...
                if (row < 0 || row > 5) {
                    //wrongRow =false;
                    assertTrue("The row of position in the array EDGE_CASE_PLACEMENTS line 2 is legal !",
                            true);
                }
                //boolean illegalCol =true;
                if (col < 0 || col > 5) {
                    //wrongCol =false;
                    assertTrue("The column of position in the array EDGE_CASE_PLACEMENTS line 2 is legal !",
                            true);
                }
            }
